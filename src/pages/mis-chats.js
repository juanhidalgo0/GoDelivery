...
import { collection, doc, query, where, getDoc, onSnapshot } from 'firebase/firestore';
...
function startChatsListener(container, user) {
  ...
  unsubChats = onSnapshot(chatsQuery, async (snap) => {
    ...
    // Sort by lastMessageAt or lastActivityAt descending
    orderChats.sort((a, b) => {
      const timeA = a.lastMessageAt?.toDate?.() || a.lastActivityAt?.toDate?.() || 0;
      const timeB = b.lastMessageAt?.toDate?.() || b.lastActivityAt?.toDate?.() || 0;
      return timeB - timeA;
    });
...
function renderChats(container, user) {
  ...
  let html = '';
  const validOrderChats = currentOrderChats.filter(c => c.order);

  // 1. Render Support Chat if it exists
  ...
  // 2. Render Order Chats
  if (validOrderChats.length > 0) {
    validOrderChats.forEach(chat => {
  ...
  if (!currentSupportChat && validOrderChats.length === 0) {
    html = `
      <div style="text-align: center; padding: 60px 20px; color: var(--color-text-tertiary); flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%;">
        ...
        <p style="margin: 0; font-size: 12.5px; font-weight: 500; line-height: 1.4;">Tus conversaciones de soporte y pedidos aparecerán aquí.</p>
      </div>
    `;
  }
