      ...
      <div class="panel-page" style="display:flex;flex-direction:column;height:100dvh;overflow:hidden;">
        <div style="position:sticky;top:0;z-index:100;display:flex;align-items:center;gap:14px;padding:16px 20px;background:var(--color-surface);border-bottom:1px solid var(--color-border-light);box-shadow:0 2px 12px rgba(0,0,0,0.08);flex-shrink:0;">
          ...
          <div style="flex:1;min-width:0;">
            <h1 style="font-family:var(--font-display);font-weight:800;font-size:18px;color:var(--color-text);margin:0;line-height:1.2;">Comisiones y Finanzas</h1>
            <p style="font-size:12px;color:var(--color-text-tertiary);margin:2px 0 0;">Resumen financiero de ${comercioData.name}</p>
          </div>
        </div>
...
function filterAndRenderOrders(orders, filterValue) {
  ...
  // Improved calculation with backward compatibility
  const pending = filtered.reduce((sum, o) => {
    const isPending = !o.commissionStatus || o.commissionStatus === 'pending';
    return sum + (isPending ? (o.commissionAmount || 0) : 0);
  }, 0);
  
  const paid = filtered.reduce((sum, o) => sum + (o.commissionStatus === 'paid' ? (o.commissionAmount || 0) : 0), 0);

  summary.style.display = 'block';
  summary.innerHTML = `
    <div class="admin-stats-grid" style="grid-template-columns: 1fr 1fr; gap: var(--space-4);">
      <div class="stat-card finance-summary-card pending" style="padding: var(--space-5); border-radius: var(--radius-2xl); position: relative; overflow: hidden; background: var(--color-bg-secondary); border: 1px solid var(--color-border); box-shadow: var(--shadow-sm);">
        <div style="position: relative; z-index: 2;">
          <div style="font-size: 24px; font-weight: 800; color: var(--color-primary); margin-bottom: 4px; letter-spacing: -0.5px;">$${pending.toLocaleString('es-AR')}</div>
          <div style="font-size: 11px; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 1px;">Pendiente</div>
        </div>
        <div style="position: absolute; right: -10px; bottom: -10px; opacity: 0.05; transform: rotate(-15deg); color: var(--color-primary);">
          ${icon('zap', 64)}
        </div>
      </div>
      
      <div class="stat-card finance-summary-card paid" style="padding: var(--space-5); border-radius: var(--radius-2xl); position: relative; overflow: hidden; background: var(--color-bg-secondary); border: 1px solid var(--color-border); box-shadow: var(--shadow-sm);">
        <div style="position: relative; z-index: 2;">
          <div style="font-size: 24px; font-weight: 800; color: #10B981; margin-bottom: 4px; letter-spacing: -0.5px;">$${paid.toLocaleString('es-AR')}</div>
          <div style="font-size: 11px; font-weight: 700; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 1px;">Pagado</div>
        </div>
        <div style="position: absolute; right: -10px; bottom: -10px; opacity: 0.05; transform: rotate(-15deg); color: #10B981;">
          ${icon('checkCircle', 64)}
        </div>
      </div>
    </div>
        ...
        return `
          <div class="finance-list-item" style="padding: var(--space-4); display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-xl); transition: all 0.2s ease;">
            <div style="display: flex; align-items: center; gap: var(--space-4); min-width: 0;">
              <div style="width: 48px; height: 48px; background: ${isPending ? 'rgba(227, 27, 35, 0.1)' : 'rgba(16, 185, 129, 0.1)'}; border-radius: 14px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; color: ${isPending ? 'var(--color-primary)' : '#10B981'};">
                ${icon(isPending ? 'package' : 'checkCircle', 24)}
              </div>
            ...
            <div style="text-align: right; flex-shrink: 0;">
              <div style="font-weight: 900; font-size: 17px; color: var(--color-text); margin-bottom: 2px; letter-spacing: -0.5px;">
                $${(o.commissionAmount || 0).toLocaleString('es-AR')}
              </div>
              <div style="font-size: 10px; font-weight: 800; color: ${isPending ? 'var(--color-primary)' : '#10B981'}; text-transform: uppercase; letter-spacing: 0.5px;">
