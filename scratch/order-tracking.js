...
const parseCoords = (c) => {
  ...
  if (lat === null && c.lat !== void 0) lat = parseFloat(c.lat);
  if (lng === null && c.lng !== void 0) lng = parseFloat(c.lng);
  if (lat === null && c.latitude !== void 0) lat = parseFloat(c.latitude);
  if (lng === null && c.longitude !== void 0) lng = parseFloat(c.longitude);
      ...
      <div style="position:absolute; top:calc(82px + env(safe-area-inset-top, 0px)); right:16px; z-index:100; display:flex; flex-direction:column; align-items:flex-end; gap:10px; pointer-events:auto;">
        <div style="display:flex; align-items:center; gap:10px;">
          <div id="v5-driver-map-tip-badge" style="background:#10b981; color:white; font-size:12.5px; font-weight:900; padding:10px 16px; border-radius:14px; border:1px solid rgba(255,255,255,0.25); white-space:nowrap; box-shadow:0 4px 14px rgba(16,185,129,0.35); display:none; align-items:center; gap:6px; font-family:system-ui, -apple-system, sans-serif;">
            \u{1F4B5} Propina: <span id="v5-driver-map-tip-value">$0</span>
          </div>
  ...
  const unsub = onSnapshot(doc(db, "orders", orderId), (snapshot) => {
    ...
    if (order.comercioId && !order.isFavor && !order.isTrip) {
      ...
      if (window.commerceCache[order.comercioId]) {
        ...
        import("firebase/firestore").then(async ({ getDoc, doc: doc2 }) => {
          try {
            const comSnap = await getDoc(doc2(db, "comercios", order.comercioId));
            if (comSnap.exists()) {
    ...
    if (isDriverView) {
      setTimeout(async () => {
        try {
          ...
          const { updateDoc, addDoc, doc: doc2, serverTimestamp: serverTimestamp2, collection: collection2 } = await import("firebase/firestore");
          const pickupBtn = document.getElementById("v5-driver-pickup-btn");
          if (pickupBtn) {
            pickupBtn.onclick = () => {
              showConfirm({
                title: order.isTrip ? "\xBFConfirmar Inicio de Viaje?" : "\xBFConfirmar Retiro?",
                message: order.isTrip ? "Confirm\xE1 que el pasajero ya est\xE1 a bordo para iniciar el trayecto." : "Asegurate de haber recibido todos los productos del local.",
                confirmText: order.isTrip ? "Iniciar Viaje" : "S\xED, retirar",
                onConfirm: async () => {
            ...
            notifyBtn.onclick = () => {
              if (order.isFavor && (order.favorType === "compra" || order.favorType === "pagodeservicios") && !order.subtotal) {
                import("../components/toast.js").then((m) => m.showToast(
                  order.favorType === "pagodeservicios" ? "\u26A0\uFE0F Debes ingresar el valor de las facturas antes de avisar que est\xE1s afuera" : "\u26A0\uFE0F Debes ingresar el valor de los productos antes de avisar que est\xE1s afuera",
                  "warning"
                ));
                showEditFavorPriceModal(order, true);
                return;
              ...
              order.isAtDoor = true;
              updateUI(order);
              updateDoc(doc2(db, "orders", order.id), {
                isAtDoor: true,
                atDoorAt: serverTimestamp2()
              ...
              if (order.userId) {
                ...
                addDoc(collection2(db, "users", order.userId, "notifications"), {
          ...
          if (whatsappBtn) {
            whatsappBtn.onclick = () => {
              ...
              if (!rawPhone) {
                import("../components/toast.js").then((t) => t.showToast("El cliente no posee un n\xFAmero de tel\xE9fono configurado.", "warning"));
                return;
              }
...
function updateUI(order, isDriverViewOverride = false) {
  ...
  if (headerDriverCard) {
    ...
    const hasCommerce2 = !isTripOrFavor && (order.comercioId || order.comercioName);
    const showDriverInfo = order.driverId && (isDelivering || order.isAtDoor || isCompleted || isCancelled);
    if (showDriverInfo) {
  ...
  let titleText = "";
  if (order.isTrip) {
    titleText = isCompleted ? "\xA1Viaje Finalizado!" : isCancelled ? "Viaje Cancelado" : order.isAtDoor ? isDriverView ? "\xA1Llegaste al destino!" : "\xA1El chofer est\xE1 en la puerta!" : isDelivering ? "Viaje en curso (Pasajero a bordo)" : !order.driverId ? "Buscando chofer..." : "El chofer va hacia tu ubicaci\xF3n";
  } else if (order.isFavor) {
    titleText = isCompleted ? "\xA1Favor Finalizado!" : isCancelled ? "Favor Cancelado" : order.isAtDoor ? isDriverView ? "\xA1Est\xE1s en la puerta!" : "\xA1El repartidor est\xE1 en la puerta!" : isDelivering ? "" : order.driverId ? "" : "";
    const hasCommerce2 = !order.isTrip && !order.isFavor && (order.comercioId || order.comercioName);
    titleText = isCompleted ? "\xA1Pedido Finalizado!" : isCancelled ? "Pedido Cancelado" : order.isAtDoor ? isDriverView ? "\xA1Est\xE1s en la puerta!" : "\xA1El repartidor est\xE1 en la puerta!" : isDelivering ? "El repartidor va hacia vos" : normalizedStatus === "pending" ? hasCommerce2 ? "" : "Esperando confirmaci\xF3n" : normalizedStatus === "ready" ? order.driverId ? order.isGoCash ? "Yendo a tu ubicaci\xF3n" : "" : "Buscando repartidor..." : "Preparando tu pedido";
  }
  let subtitleHtml = "";
    ...
    if (isDriverView) {
      ...
      subtitleHtml = `
        <p class="v5-status-subtitle" style="font-size: 13px; color: #d97706; margin: 6px 0 0 0; font-weight: 700; line-height: 1.4;">
          Por favor, sal\xED a recibir ${order.isTrip ? "al chofer" : "tu pedido"} para evitar demoras.
        </p>
      `;
    }
    ...
    if (order.isFavor && normalizedStatus === "pending") {
      ...
      subtitleHtml = hasCommerce ? "" : `
        <p class="v5-status-subtitle" style="font-size: 13px; color: var(--color-text-secondary); margin: 6px 0 0 0; font-weight: 550; line-height: 1.4;">
          Por favor espera a que el comercio confirme tu pedido, tomar\xE1 solo un momento.
        </p>
      `;
    } else if (normalizedStatus === "confirmed" || normalizedStatus === "preparing") {
      subtitleHtml = `
        <p class="v5-status-subtitle" style="font-size: 13px; color: var(--color-text-secondary); margin: 6px 0 0 0; font-weight: 550; line-height: 1.4;">
          El comercio acept\xF3 tu pedido y se encuentra prepar\xE1ndolo.
        </p>
      `;
    } else {
  ...
  container.innerHTML = `
    ...
    ${order.isTrip ? `
      <div style="background:var(--color-bg-secondary); padding:14px; border-radius:18px; border:1px solid var(--color-border-light); margin-top:4px; display:flex; flex-direction:column; gap:10px;">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="font-size:16px;">${order.tripType === "moto" ? "\u{1F3CD}\uFE0F" : "\u{1F697}"}</span>
          <span style="font-size:12px; font-weight:800; color:var(--color-text-primary); text-transform:capitalize;">Veh\xEDculo: ${order.tripType === "moto" ? "Moto" : "Auto"}</span>
        </div>
    ...
    ${!isDriverView && isDelivering && !order.isTrip ? `
      <div class="v5-cta-code"><span>C\xD3DIGO DE ENTREGA</span><span class="v5-code-val">${order.verificationCode}</span></div>
    ` : ""}
    ...
    ${!isDriverView && order.driverId && (order.paymentMethod === "mercadopago" || order.paymentMethod === "transferencia" || order.paymentMethod === "transfer" || order.paymentMethod && order.paymentMethod.toString().toLowerCase().includes("transf")) ? `
      ...
      <div style="background:var(--color-surface); border:1px solid var(--color-border-light); border-radius:16px; padding:12px 14px; margin-top:2px; display:flex; align-items:center; justify-content:space-between; box-shadow:0 2px 8px rgba(0,0,0,0.02); width:100%; box-sizing:border-box;">
        <div style="display:flex; flex-direction:column; min-width:0; flex:1; margin-right:12px;">
          ...
          <span style="font-size:14px; font-weight:800; color:var(--color-text-primary); text-overflow:ellipsis; overflow:hidden; white-space:nowrap;" id="v5-driver-alias-val">${order.driverAlias || "Esperando asignaci\xF3n..."}</span>
        </div>
        ${order.driverAlias ? `
    ...
    ${isDriverView && !isCompleted && !isCancelled ? `
      <!-- Botones de Acci\xF3n del Repartidor -->
        ...
        <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid var(--color-border-light); display: flex; flex-direction: column; align-items: flex-end; width: 100%;">
          ...
          ${order.pointsRedeemed > 0 ? `
            <div style="display: flex; align-items: center; gap: 5px; font-size: 10px; font-weight: 750; color: var(--color-text-secondary); opacity: 0.85; margin-top: 4px;">
              ${icon("goPointsLogo", 11)} Go Points canjeados ser\xE1n reintegrados
            </div>
          ` : ""}
        </div>
      ` : ""}
  ...
  document.getElementById("v5-copy-alias-btn")?.addEventListener("click", () => {
    if (order.driverAlias) {
      navigator.clipboard.writeText(order.driverAlias).then(() => {
        showToast("Alias copiado al portapapeles \u{1F4CB}", "success");
      }).catch(() => {
  ...
  document.getElementById("v5-cancel-order-btn")?.addEventListener("click", () => {
    ...
    const pointsText = redeemedPoints > 0 ? `Los <b>${redeemedPoints} Go Points</b> canjeados ser\xE1n reintegrados de forma autom\xE1tica e inmediata a tu cuenta.` : "Los <b>Go Points</b> canjeados en esta compra (si los hubiere) ser\xE1n reintegrados de forma autom\xE1tica e inmediata a tu cuenta.";
    const confirmMessage = order.isTrip ? "\xBFEst\xE1s seguro de que deseas cancelar este viaje? Se cancelar\xE1 la solicitud de inmediato." : `
      \xBFEst\xE1s seguro de que deseas cancelar este pedido? Se notificar\xE1 al comercio de inmediato.
      <br><br>
      ...
      onConfirm: async () => {
        try {
          await runTransaction(db, async (transaction) => {
            ...
            const rawStatus2 = (orderData.status || "").toString().toLowerCase();
            const allowedCancelStatuses = orderData.isTrip ? ["ready", "preparing", "confirmed", "pending"] : ["pendiente", "pending"];
            if (!allowedCancelStatuses.includes(rawStatus2)) {
              throw orderData.isTrip ? "Un chofer ya inici\xF3 el viaje o este ya fue cancelado." : "El comercio ya est\xE1 preparando tu pedido o este ya fue cancelado.";
            }
          ...
          showToast("Pedido cancelado con \xE9xito", "success");
        } catch (err) {
...
async function updateRoute(start, end) {
  ...
  if (!lastRouteStartCoords || !lastRouteEndCoords) {
    ...
    if (timeElapsed >= 1e4 && (startMoved >= 12 || endMoved >= 8)) {
      shouldFetch = true;
    }
  }
  if (!shouldFetch) {
...
async function calculatePredictiveETA(order) {
  ...
  if (order.comercioId && !isFavor) {
    try {
      ...
      pastOrders.forEach((p) => {
        const durationMin = (p.readyAt - p.confirmedAt) / 6e4;
        ...
        if (p.readyAt && p.pickedUpAt) {
          ...
          const delayMin = (pickedUpTime - p.readyAt) / 6e4;
  ...
  if (rawStatus === "confirmed" || rawStatus === "preparando") {
    if (order.confirmedAt?.toDate) {
      ...
      const elapsedMin = Math.floor((now - confirmedTime) / 6e4);
      prepTimeRemaining = Math.max(2, prepTime - elapsedMin);
    }
  } else if (rawStatus === "ready" || rawStatus === "listo" || rawStatus === "en camino" || rawStatus === "delivering") {
    prepTimeRemaining = 0;
  }
  let riderToStoreTime = 5;
  let storeToCustomerTime = 8;
  ...
  if (order.isFavor) {
    ...
    const shoppingOrPickupTime = isShopping ? 15 : 5;
...
export function showPointsEarnedModal(order) {
  ...
  const hasPoints = points !== void 0 && points !== null;
  ...
  const modalContent = `
    <div style="padding: 24px 20px; text-align: center; color: var(--color-text-primary); font-family: var(--font-body); display: flex; flex-direction: column; align-items: center; gap: 16px;">
      ...
      <div style="margin-top: 8px;">
        <h3 style="font-family: var(--font-display); font-size: 22px; font-weight: 950; margin: 0; letter-spacing: -0.5px;">\xA1Pedido Entregado!</h3>
        <p style="font-size: 13px; color: var(--color-text-secondary); margin: 6px 0 0 0; font-weight: 600; line-height: 1.45;">
          \xA1Tu pedido #${(order.orderId || order.id || "").toString().slice(-4).toUpperCase()} lleg\xF3 con \xE9xito!
        </p>
      </div>
      ...
      <div id="modal-points-multiplier-container" style="font-size: 11.5px; color: var(--color-text-tertiary); line-height: 1.5; text-align: center; max-width: 280px; font-weight: 600;">
        ${hasPoints ? `Multiplicador de nivel <strong style="color: ${lvlInfo.color}; font-weight: 900;">${lvlInfo.name}</strong> activo: <strong style="color: var(--color-primary); font-weight: 900;">${order.appliedMultiplier || 1}x puntos</strong>.` : ""}
      </div>
      ...
      <div id="modal-points-referral-container" style="width: 100%;">
        ${order.referredRewardGranted ? `
          <div style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.08) 0%, rgba(245, 158, 11, 0.02) 100%); border: 1.5px dashed rgba(245, 158, 11, 0.4); border-radius: 20px; padding: 16px; width:100%; box-sizing:border-box; text-align:left; display:flex; gap:12px; align-items:flex-start; margin-top: 4px;">
            <span style="font-size:24px; animation: scale-pulse 2s infinite;">\u{1F381}</span>
            <div>
              <strong style="font-size:13px; color:#d97706; display:block; margin-bottom:2px;">\xA1Bono de Referido Acreditado!</strong>
              <span style="font-size:11.5px; color:var(--color-text-secondary); line-height:1.45; display:block;">
                Por haber ingresado con la invitaci\xF3n de tu amigo y completar tu primer pedido, te regalamos <strong>${order.referralBonusAmount || 500} GO Points extra</strong>. \xA1Disfrutalos!
              </span>
            </div>
          </div>
        ` : ""}
  ...
  `;
  showModal({
    title: "\u{1F389} \xA1Pedido Entregado!",
    height: "auto",
...
export function updatePointsModalValues(order) {
  ...
  if (!pointsContainer || order.pointsEarned === void 0) return;
  const points = order.pointsEarned;
  ...
  const lvlInfo = levelMap[order.userLevel || "bronce"] || levelMap.bronce;
  const formatPrice2 = (val) => `$${Number(val).toLocaleString("es-AR", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;
  ...
  if (multiplierContainer) {
    multiplierContainer.innerHTML = `
      Multiplicador de nivel <strong style="color: ${lvlInfo.color}; font-weight: 900;">${lvlInfo.name}</strong> activo: <strong style="color: var(--color-primary); font-weight: 900;">${order.appliedMultiplier || 1}x puntos</strong>.
    `;
  }
  if (referralContainer && order.referredRewardGranted) {
    referralContainer.innerHTML = `
      <div style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.08) 0%, rgba(245, 158, 11, 0.02) 100%); border: 1.5px dashed rgba(245, 158, 11, 0.4); border-radius: 20px; padding: 16px; width:100%; box-sizing:border-box; text-align:left; display:flex; gap:12px; align-items:flex-start; margin-top: 4px;">
        <span style="font-size: 24px;">\u{1F381}</span>
        <div>
          <strong style="font-size: 13px; color: #d97706; display: block; margin-bottom: 2px;">\xA1Bono de Referido Acreditado!</strong>
          <span style="font-size: 11.5px; color: var(--color-text-secondary); line-height: 1.45; display: block;">
            Por haber ingresado con la invitaci\xF3n de tu amigo y completar tu primer pedido, te regalamos <strong>${order.referralBonusAmount || 500} GO Points extra</strong>. \xA1Disfrutalos!
...
if (!window._clientTimerInterval) {
  window._clientTimerInterval = setInterval(() => {
    document.querySelectorAll(".client-queue-timer").forEach((el) => {
      ...
      if (!offeredAt) return;
      const elapsedSec = Math.floor((Date.now() - offeredAt) / 1e3);
    ...
    document.querySelectorAll(".scheduled-countdown-timer").forEach((el) => {
      ...
      const diffSec = Math.floor((targetTime - now) / 1e3);
...
window.openOrderDetailsModal = function(order) {
  ...
  const detailsHtml = `
    ...
    ${o.isTrip ? `
      <div style="background:var(--color-bg-secondary); padding:16px; border-radius:20px; border:1px solid var(--color-border-light); margin-bottom:14px; text-align: left; display:flex; flex-direction:column; gap:10px;">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="font-size:16px;">${o.tripType === "moto" ? "\u{1F3CD}\uFE0F" : "\u{1F697}"}</span>
          <span style="font-size:13px; font-weight:800; color:var(--color-text-primary); text-transform:capitalize;">Veh\xEDculo: ${o.tripType === "moto" ? "Moto" : "Auto"}</span>
        </div>
  ...
  `;
  container.innerHTML = `
    <div id="v5-details-modal-backdrop" onclick="window.closeOrderDetailsModal()" style="position:fixed; inset:0; background:rgba(0,0,0,0.25); backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); z-index:9998; opacity:0; transition:opacity 0.25s ease;"></div>
      ...
      <div style="padding: 22px; display:flex; flex-direction:column; gap:11px; font-size:13px; font-weight:700;">
        ...
        ${rainSurcharge > 0 ? `
          <div style="display:flex; justify-content:space-between; color:#009EE3;">
            <span>Recargo por Lluvia \u{1F327}\uFE0F:</span>
...
function getStepCircleContent(order, stepIdx, activeIdx, isFavor) {
  ...
  if (stepIdx === activeIdx) {
    if (isFavor) {
      ...
      switch (stepIdx) {
        case 0:
          return `<img src="/go-hourglass.png" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; animation: float 2.5s infinite ease-in-out; display:block;" />`;
        case 1:
          return `<img src="/go-thumbs-up.png" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; animation: bounce-slow 1.5s infinite; display:block;" />`;
        case 2:
          return `<img src="/go-bag.png?v=6" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; animation: bounce-slow 1.5s infinite; display:block;" />`;
        case 3:
          return `<img src="/go-box.png" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; animation: bounce-slow 1.5s infinite; display:block;" />`;
        case 4:
          return `<img src="/go-delivery-moto.png?v=2" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; animation: wiggle 1s infinite; display:block;" />`;
      }
    }
  }
...
function getTripStepCircleContent(order, stepIdx, activeIdx) {
  ...
  if (stepIdx === activeIdx) {
    switch (stepIdx) {
      case 0:
        return `<img src="/go-hourglass.jpg" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; border-radius:50%; animation: float 2.5s infinite ease-in-out; display:block;" />`;
      case 1:
        return `<img src="/go-thumbs-up.jpg" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; border-radius:50%; animation: bounce-slow 1.5s infinite; display:block;" />`;
      case 2:
        return `<img src="${order.tripType === "moto" ? "/go-scooter.jpg" : "/go-car.jpg"}" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; border-radius:50%; animation: bounce-slow 1.5s infinite; display:block;" />`;
      case 3:
        return `<img src="${order.tripType === "moto" ? "/go-scooter.jpg" : "/go-car.jpg"}" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; border-radius:50%; animation: wiggle 1.2s infinite; display:block;" />`;
      case 4:
        return `<img src="/go-thumbs-up.jpg" class="step-icon-animated" style="width:20px; height:20px; object-fit:contain; border-radius:50%; animation: bounce-slow 1.5s infinite; display:block;" />`;
    }
  }
...
function getStatusBannerHTML(order, normalizedStatus, isDriverView = false) {
  if (isDriverView) {
    const isCompleted2 = normalizedStatus === "completed";
    const isCancelled2 = normalizedStatus === "cancelled";
    if (!isCompleted2 && !isCancelled2) {
      let bannerIcon2 = "";
      let bannerTitle2 = "";
      let bannerText2 = "";
      let bannerColor2 = "";
      ...
      if (!isDelivering) {
        ...
        bannerIcon2 = `<img src="/go-bag.png?v=6" style="width:54px; height:54px; object-fit:contain;" />`;
        bannerTitle2 = "RETIRA EL PEDIDO EN:";
        bannerText2 = `<strong style="font-size:15px; display:block; margin-top:2px;">${commerceName}</strong>`;
        bannerColor2 = "#0284c7";
        bannerBg2 = "rgba(2, 132, 199, 0.05)";
        bannerBorder2 = "rgba(2, 132, 199, 0.15)";
      } else {
        const deliveryAddress = order.deliveryAddress || "Direcci\xF3n del cliente";
        ...
        bannerIcon2 = `<img src="/go-delivery-moto.png?v=2" style="width:54px; height:54px; object-fit:contain; margin: -6px 0;" />`;
        bannerTitle2 = "DIR\xCDGETE A ENTREGAR EL PEDIDO:";
        bannerText2 = `
          <strong style="font-size:15px; display:block; margin-top:2px;">${deliveryAddress}</strong>
          ${addressNotes}
        `;
        bannerColor2 = "#10b981";
        bannerBg2 = "rgba(16, 185, 129, 0.05)";
        bannerBorder2 = "rgba(16, 185, 129, 0.15)";
      }
      return `
        <div style="background:${bannerBg2}; border:1px solid ${bannerBorder2}; border-radius:24px; padding:16px 20px; display:flex; align-items:center; gap:16px; width:100%; box-sizing:border-box; margin-bottom:12px; transition:all 0.3s ease;">
          <div style="width:56px; height:56px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
            ${bannerIcon2}
          </div>
          <div style="flex:1; min-width:0; text-align:left;">
            <div style="font-size:10px; font-weight:800; color:${bannerColor2}; text-transform:uppercase; letter-spacing:0.05em; line-height:1.2;">${bannerTitle2}</div>
  ...
  if (order.isTrip) {
    if (normalizedStatus === "pending") {
      ...
      bannerTitle = "\xA1Buscando Chofer!";
      bannerText = "Estamos buscando al chofer m\xE1s cercano para iniciar tu viaje.";
      bannerColor = "#d97706";
      ...
      if (activeIdx === 1) {
        ...
        bannerTitle = "\xA1Chofer Asignado!";
        bannerText = "El chofer se est\xE1 preparando para ir a buscarte.";
        bannerColor = "#0284c7";
        bannerBg = "rgba(2, 132, 199, 0.05)";
        bannerBorder = "rgba(2, 132, 199, 0.15)";
      } else if (activeIdx === 2) {
        bannerIcon = `<img src="${order.tripType === "moto" ? "/go-scooter.jpg" : "/go-car.jpg"}" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
        bannerTitle = "\xA1Chofer en camino!";
        ...
        bannerIcon = `<img src="${order.tripType === "moto" ? "/go-scooter.jpg" : "/go-car.jpg"}" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
        bannerTitle = "\xA1Viaje en Curso!";
        bannerText = "Est\xE1s en viaje con el conductor hacia tu destino.";
        bannerColor = "#10b981";
        ...
        bannerTitle = "\xA1Llegaste!";
        bannerText = "Has arribado con \xE9xito a tu destino final.";
        bannerColor = "#10b981";
        bannerBg = "rgba(16, 185, 129, 0.05)";
        bannerBorder = "rgba(16, 185, 129, 0.15)";
      }
    ...
    if (normalizedStatus === "pending") {
      ...
      bannerTitle = isMandado ? "\xA1Mandado Solicitado!" : favorType === "gocash" ? "\xA1Retiro Solicitado!" : "\xA1Env\xEDo Solicitado!";
      bannerText = isMandado ? "Estamos asignando un repartidor para tu mandado." : favorType === "gocash" ? "Registrando tu solicitud de retiro de dinero." : "Registrando tu solicitud de env\xEDo de encomienda.";
      bannerColor = "#d97706";
      bannerBg = "rgba(217, 119, 6, 0.05)";
      bannerBorder = "rgba(217, 119, 6, 0.15)";
    } else if (normalizedStatus === "confirmed" || normalizedStatus === "preparing") {
      if (order.driverId) {
        ...
        bannerText = "Tu repartidor est\xE1 yendo a buscar tu pedido";
        bannerColor = "#0284c7";
        bannerBg = "rgba(2, 132, 199, 0.05)";
        bannerBorder = "rgba(2, 132, 199, 0.15)";
      } else {
        ...
        bannerText = "Buscando al repartidor m\xE1s cercano disponible.";
        bannerColor = "#ef4444";
      ...
      bannerText = isMandado ? "El repartidor est\xE1 yendo al punto de compra del mandado." : favorType === "gocash" ? "El repartidor est\xE1 yendo a buscar el dinero en efectivo." : "El repartidor est\xE1 yendo al punto de retiro del paquete.";
      bannerColor = "#8B5CF6";
      bannerBg = "rgba(139, 92, 246, 0.05)";
      bannerBorder = "rgba(139, 92, 246, 0.15)";
      ...
      bannerTitle = isMandado ? "\xA1Mandado en camino!" : favorType === "gocash" ? "\xA1Efectivo en camino!" : "\xA1Paquete en camino!";
      bannerText = isMandado ? "El repartidor va hacia tu ubicaci\xF3n con la compra." : favorType === "gocash" ? "El repartidor lleva el dinero a tu ubicaci\xF3n." : "El repartidor retir\xF3 la encomienda y va hacia el destino.";
      bannerColor = "#10b981";
      bannerBg = "rgba(16, 185, 129, 0.05)";
      bannerBorder = "rgba(16, 185, 129, 0.15)";
    } else if (order.isAtDoor) {
      bannerIcon = `<img src="/go-thumbs-up.jpg" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
      bannerTitle = "\xA1Lleg\xF3 a tu puerta!";
      bannerText = isMandado ? "El repartidor lleg\xF3 con tus compras. \xA1Sal a recibirlo!" : favorType === "gocash" ? "El repartidor lleg\xF3 con el dinero. \xA1Sal a recibirlo!" : "El repartidor est\xE1 afuera con la encomienda. \xA1Sal a recibirlo!";
      bannerColor = "#f59e0b";
    ...
    if (normalizedStatus === "pending") {
      ...
      bannerTitle = "\xA1Esperando Confirmaci\xF3n!";
      bannerText = "El comercio est\xE1 revisando tu pedido para confirmarlo.";
      bannerColor = "#d97706";
      bannerBg = "rgba(217, 119, 6, 0.05)";
      bannerBorder = "rgba(217, 119, 6, 0.15)";
    } else if (normalizedStatus === "confirmed") {
      bannerIcon = `<img src="/go-thumbs-up.png" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
      bannerTitle = "\xA1Pedido Aprobado!";
      bannerText = "El comercio acept\xF3 tu compra y se dispone a prepararla.";
      bannerColor = "#0284c7";
      bannerBg = "rgba(2, 132, 199, 0.05)";
      bannerBorder = "rgba(2, 132, 199, 0.15)";
      ...
      bannerTitle = "\xA1Preparando tu pedido!";
      if (order.isScheduled && order.scheduledFor) {
        const targetMillis = order.scheduledFor.toMillis ? order.scheduledFor.toMillis() : order.scheduledFor.seconds * 1e3;
        bannerText = `
        ...
        bannerText = "El comercio est\xE1 preparando tus productos con cuidado.";
      }
      bannerColor = "#8B5CF6";
      bannerBg = "rgba(139, 92, 246, 0.05)";
      bannerBorder = "rgba(139, 92, 246, 0.15)";
      ...
      if (!order.driverId) {
        bannerIcon = `<img src="/go-hourglass.png" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
        bannerTitle = "Buscando Repartidor";
        bannerText = "Tu pedido est\xE1 listo. Estamos buscando un repartidor disponible.";
        bannerColor = "#ef4444";
        bannerBg = "rgba(239, 68, 68, 0.05)";
        bannerBorder = "rgba(239, 68, 68, 0.15)";
      } else {
        bannerIcon = `<img src="/go-box.png" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
        bannerTitle = "\xA1Pedido Listo!";
        bannerText = "Tu pedido est\xE1 listo. El repartidor est\xE1 retir\xE1ndolo.";
        bannerColor = "#10b981";
        bannerBg = "rgba(16, 185, 129, 0.05)";
        bannerBorder = "rgba(16, 185, 129, 0.15)";
      }
      ...
      bannerTitle = "\xA1En camino!";
      bannerText = "El repartidor est\xE1 yendo a tu casa.";
      bannerColor = "#10b981";
      bannerBg = "rgba(16, 185, 129, 0.05)";
      bannerBorder = "rgba(16, 185, 129, 0.15)";
    } else if (order.isAtDoor) {
      bannerIcon = `<img src="/go-thumbs-up.png" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
      bannerTitle = "\xA1Lleg\xF3 a tu puerta!";
      bannerText = "El repartidor se encuentra afuera. \xA1Por favor sal a recibirlo!";
      bannerColor = "#f59e0b";
      bannerBg = "rgba(245, 158, 11, 0.05)";
      bannerBorder = "rgba(245, 158, 11, 0.15)";
    } else {
      bannerIcon = `<img src="/go-hourglass.png" style="width:38px; height:38px; object-fit:contain; mix-blend-mode:multiply;" />`;
      bannerTitle = "Buscando Repartidor";
      bannerText = "Estamos buscando al repartidor m\xE1s cercano para tu entrega.";
      bannerColor = "#ef4444";
      bannerBg = "rgba(239, 68, 68, 0.05)";
      bannerBorder = "rgba(239, 68, 68, 0.15)";
    }
...
window.checkAndShowCompletedChallenges = async function() {
  ...
  const user = getState2().user;
  if (!user?.uid) return;
  try {
    const { collection: collection2, getDocs: getDocs2, query: query2, where: where2, doc: doc2, updateDoc } = await import("firebase/firestore");
    const challengesRef = collection2(db, "users", user.uid, "challenges");
    const q = query2(challengesRef, where2("completed", "==", true), where2("modalShown", "==", false));
    const snap = await getDocs2(q);
    if (!snap.empty) {
      const docSnap = snap.docs[0];
      const challenge = docSnap.data();
      await updateDoc(doc2(db, "users", user.uid, "challenges", docSnap.id), {
        modalShown: true
      });
      const { showModal: showModal2 } = await import("../components/modal.js");
      showModal2({
        title: "\u{1F3C6} \xA1Desaf\xEDo Completado!",
        height: "auto",
        content: `
          <div style="padding: 24px 20px; text-align: center; color: var(--color-text-primary); font-family: var(--font-body); display: flex; flex-direction: column; align-items: center; gap: 16px;">
            ...
            <div>
              <h3 style="font-family: var(--font-display); font-size: 20px; font-weight: 950; margin: 0; letter-spacing: -0.5px;">\xA1Felicidades!</h3>
              <p style="font-size: 13.5px; color: var(--color-text-secondary); margin: 6px 0 0 0; font-weight: 600; line-height: 1.45;">
                Completaste el desaf\xEDo semanal: <br><strong style="color:var(--color-text-primary); font-size: 15px;">${challenge.title}</strong>
              </p>
            </div>
            <div style="background: var(--color-bg-secondary); border: 1.5px solid var(--color-border-light); border-radius: 20px; padding: 18px 24px; width: 100%; display: flex; flex-direction: column; align-items: center; gap: 4px; box-shadow: var(--shadow-sm);">
