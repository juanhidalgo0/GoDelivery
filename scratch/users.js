...
export async function renderAdminUsers() {
    ...
    <div class="panel-page" style="display:flex;flex-direction:column;height:100dvh;width:100%;position:fixed;top:0;left:0;z-index:1000;overflow:hidden;background:var(--color-bg-secondary);">
      ...
      <div style="position:sticky;top:0;z-index:100;display:flex;align-items:center;gap:14px;padding:16px 20px;background:var(--color-primary);flex-shrink:0;position:relative;overflow:hidden;box-shadow:0 4px 12px rgba(var(--color-primary-rgb),0.2);">
        ...
        <div style="flex:1;min-width:0;position:relative;z-index:2;">
          <h1 style="font-family:var(--font-display);font-weight:900;font-size:20px;color:white;margin:0;line-height:1.2;letter-spacing:-0.02em;display:flex;align-items:center;gap:8px;">
            <span>Gesti\xF3n de Usuarios</span>
        ...
        <button id="users-header-reload-btn" style="background:rgba(255,255,255,0.15); border:none; width:40px; height:40px; border-radius:12px; cursor:pointer; display:flex; align-items:center; justify-content:center; color:white; transition:all 0.2s; position:relative; z-index:2;" title="Recargar p\xE1gina">
          ${icon("refresh", 22)}
        </button>
      </div>
      ...
      <div style="flex:1;overflow-y:auto;padding:16px 16px 40px;-webkit-overflow-scrolling:touch; display:flex; flex-direction:column; gap:20px;">
        ...
        <div style="display:flex; gap:10px; flex-shrink:0; width:100%; flex-wrap:wrap; margin-top:-4px;">
          ...
          <div style="flex:1; min-width:140px; display:flex; flex-direction:column; gap:4px;">
            ...
            <div style="position:relative; display:flex; align-items:center; background:var(--color-surface); border:1px solid var(--color-border-light); border-radius:12px; padding:0 12px; height:44px; box-shadow:var(--shadow-sm);">
              ...
              <select id="users-sort" style="flex:1; border:none; background:transparent; font-size:13px; font-weight:700; color:var(--color-text); outline:none; appearance:none; cursor:pointer; padding-right:20px;">
                ...
                <option value="newest">M\xE1s nuevo</option>
                <option value="rating-desc">Mayor puntuaci\xF3n</option>
                <option value="rating-asc">Menor puntuaci\xF3n</option>
              </select>
          ...
          <!-- Stars Rating Filter -->
          <div style="flex:1; min-width:140px; display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10px; font-weight:800; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.05em; padding-left:4px;">Calificaci\xF3n</label>
            <div style="position:relative; display:flex; align-items:center; background:var(--color-surface); border:1px solid var(--color-border-light); border-radius:12px; padding:0 12px; height:44px; box-shadow:var(--shadow-sm);">
              ...
              <select id="users-stars" style="flex:1; border:none; background:transparent; font-size:13px; font-weight:700; color:var(--color-text); outline:none; appearance:none; cursor:pointer; padding-right:20px;">
                ...
                <option value="none">Sin calificaci\xF3n</option>
              </select>
              <span style="position:absolute; right:12px; color:var(--color-text-tertiary); pointer-events:none; display:flex;">${icon("chevronDown", 14)}</span>
            </div>
          </div>
          ...
          <div id="users-connection-filter-container" style="flex:1; min-width:140px; display:flex; flex-direction:column; gap:4px;">
            <label style="font-size:10px; font-weight:800; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.05em; padding-left:4px;">Conexi\xF3n (Repartidores)</label>
            <div style="position:relative; display:flex; align-items:center; background:var(--color-surface); border:1px solid var(--color-border-light); border-radius:12px; padding:0 12px; height:44px; box-shadow:var(--shadow-sm);">
  ...
  document.getElementById("reset-all-drivers-debt-btn")?.addEventListener("click", () => {
    showConfirm({
      ...
      message: "\xBFEst\xE1s seguro de que deseas poner en <b>$0</b> la deuda de TODOS los repartidores de la plataforma?",
      danger: true,
      confirmText: "S\xED, resetear todo a $0",
      onConfirm: async () => {
        try {
          ...
          await batch.commit();
          showToast(`\xA1Se resete\xF3 a $0 la deuda de ${count} repartidores!`, "success");
          updateList();
        } catch (err) {
  ...
  document.getElementById("users-list")?.addEventListener("click", async (e) => {
    ...
    if (deleteBtn) {
      ...
      if (uid === currentUser.uid) {
        showToast("No pod\xE9s eliminarte a vos mismo", "warning");
        return;
      }
      showConfirm({
        title: "ELIMINAR USUARIO",
        message: "\xBFEst\xE1s seguro de que deseas eliminar permanentemente a este usuario y todos sus datos asociados de la plataforma?",
        danger: true,
        ...
        onConfirm: async () => {
          try {
            ...
            showToast("Usuario eliminado con \xE9xito", "success");
          } catch (err) {
    ...
    if (canonToggleBtn) {
      ...
      showConfirm({
        ...
        message: `\xBFQuer\xE9s ${isCurrentlyActive ? "BLOQUEAR" : "HABILITAR"} la jornada de hoy (${todayStr}) para ${targetUser.displayName}?`,
        onConfirm: async () => {
          try {
    ...
    if (canonExemptBtn) {
      ...
      showConfirm({
        title: newExemptState ? "Eximir de Canon" : "Quitar Exenci\xF3n",
        message: `\xBFQuer\xE9s ${newExemptState ? "EXIMIR permanentemente" : "REQUERIR canon"} de pago a ${targetUser.displayName}?`,
        onConfirm: async () => {
          try {
            await updateDoc(doc(db, "users", uid), { isCanonExempt: newExemptState });
            targetUser.isCanonExempt = newExemptState;
            renderUsersList(users, document.getElementById("users-search")?.value || "", currentUser, canChangeRoles, currentFilter, getSortVal(), getStarsVal(), getOsVal());
            showToast(`Exenci\xF3n actualizada para ${targetUser.displayName}`, "success");
          } catch (err) {
    ...
    if (toggleBtn && canChangeRoles) {
      ...
      if (uid === currentUser.uid && field === "isAdmin") {
        showToast("No pod\xE9s quitarte tus propios permisos de Administrador", "warning");
        return;
      }
      const newValue = field === "isChofer" ? targetUser.tripStatus !== "approved" : !targetUser[field];
      showConfirm({
        title: "Actualizar Acceso",
        message: `\xBFQuer\xE9s ${newValue ? "ACTIVAR" : "DESACTIVAR"} el permiso de <b>${field === "isChofer" ? "Chofer" : field.replace("is", "")}</b> para ${targetUser.displayName}?`,
        onConfirm: async () => {
          try {
            const updateData = {};
            if (field === "isChofer") {
              ...
              if (newValue) {
                ...
                if (!targetUser.deliveryId) {
                  ...
                  await runTransaction(db, async (t) => {
                    ...
                    const nId = (sSnap.exists() ? sSnap.data().lastDeliveryId || 1e3 : 1e3) + 1;
            ...
            if (field === "isComercio") {
              if (newValue) {
                ...
                const { query, where, getDocs: getDocs2 } = await import("firebase/firestore");
                const comsSnap = await getDocs2(query(collection(db, "comercios"), where("name", "==", trimmedName)));
                if (!comsSnap.empty) {
                  showToast(`El nombre de comercio "${trimmedName}" ya est\xE1 registrado por otra cuenta.`, "error");
                  return;
                }
    ...
    if (openChatBtn) {
      ...
      const targetUser = users.find((u) => u.uid === uid);
      if (targetUser) {
        const { collection: collection2, query, where, getDocs: getDocs2, addDoc, serverTimestamp: fServerTimestamp } = await import("firebase/firestore");
        try {
          const q = query(collection2(db, "support_chats"), where("userId", "==", uid));
          ...
          if (activeDoc) {
            ...
            const ticketNum = Math.floor(1e5 + Math.random() * 9e5);
            const newDocRef = await addDoc(collection2(db, "support_chats"), {
              ...
              lastMessageText: "Conversaci\xF3n iniciada por el administrador",
...
function renderUsersList(users, search, currentUser, canChangeRoles, filter = "all", sortVal = "none", starsVal = "all", osVal = "all", connVal = "all") {
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (u) => (u.displayName || "").toLowerCase().includes(s) || (u.email || "").toLowerCase().includes(s) || (u.goId || "").toLowerCase().includes(s) || (u.deliveryId || "").toLowerCase().includes(s)
    );
  }
  if (osVal && osVal !== "all") {
  ...
  container.innerHTML = filtered.map((u) => {
    ...
    if (u.isDelivery || u.tripStatus === "approved") {
      ...
      let lastActText = "";
      if (lastAct) {
        const diffMins = Math.floor((Date.now() - lastAct.getTime()) / 6e4);
      ...
      if (isOnline) {
        connectionBadgeHTML = `
          <span class="id-badge" style="background:rgba(34,197,94,0.1); color:#22c55e; border:1px solid rgba(34,197,94,0.2); font-weight:850; display:inline-flex; align-items:center; gap:4.5px;">
            ...
            Online${lastActText ? ` \xB7 ${lastActText}` : ""}
          </span>
        `;
      } else {
        connectionBadgeHTML = `
          <span class="id-badge" style="background:rgba(148,163,184,0.1); color:#64748b; border:1px solid rgba(148,163,184,0.2); font-weight:850; display:inline-flex; align-items:center; gap:4.5px;">
            ...
            Offline${lastActText ? ` \xB7 ${lastActText}` : ""}
          </span>
        `;
      }
    }
    ...
    const ratingBadgeHTML = avgText !== null ? `
      <span class="status-badge" data-view-ratings="${u.uid}" style="background:rgba(251,191,36,0.1); color:#d97706; border:1px solid rgba(251,191,36,0.2); cursor:pointer; user-select:none; display:flex; align-items:center; gap:4px; font-weight:800; transition:all 0.2s;" onmouseover="this.style.background='rgba(251,191,36,0.18)'" onmouseout="this.style.background='rgba(251,191,36,0.1)'">
        ${icon("star", 12)} ${avgText} (${count} ${count === 1 ? "rese\xF1a" : "rese\xF1as"})
      </span>
    ` : `
    ...
    return `
      <div class="user-card" style="position:relative; ${isMe ? "border: 1px solid var(--color-primary);" : ""}">
        ...
        <div class="user-info-row" data-view-ratings="${u.uid}" style="display:flex; gap:16px; align-items:flex-start; cursor:pointer;" title="Click para ver rese\xF1as">
          ...
          <div style="flex:1; min-width:0; padding-right: 40px;">
            <div style="font-weight:800; font-size:16px; color:var(--color-text); display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
              ${commerceData ? `\u{1F3EA} ${commerceData.name || "Tienda sin nombre"}` : u.displayName || "Sin nombre"}
            ...
            ${commerceData ? `<div style="font-size:12px; color:var(--color-text-tertiary); margin-bottom:2px; font-weight:600;">Due\xF1o/a: ${u.displayName || "Sin nombre"}</div>` : ""}
...
function showUserRatingsModal(u) {
  ...
  const count = ratings.length;
  const avg = count ? (ratings.reduce((sum, r) => sum + r.rating, 0) / count).toFixed(1) : "Sin calificaci\xF3n";
  ...
  if (count === 0) {
    reviewsListHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:40px 20px; text-align:center; color:var(--color-text-tertiary); flex:1;">
        ...
        <h4 style="font-size:16px; font-weight:800; color:var(--color-text); margin:0 0 6px;">Sin rese\xF1as a\xFAn</h4>
        <p style="font-size:12px; max-width:240px; margin:0; line-height:1.4;">Este usuario todav\xEDa no ha recibido ninguna calificaci\xF3n en la plataforma.</p>
      </div>
    `;
  } else {
    reviewsListHTML = `
      ...
      if (r.userName) {
        ...
        authorHTML = `Rese\xF1a An\xF3nima`;
      }
      return `
    ...
    <div style="display:flex; flex-direction:column; height:100%; overflow:hidden; background:var(--color-bg-secondary);">
      ...
      <div style="background:var(--color-surface); border-bottom:1px solid var(--color-border-light); padding:16px 20px; display:flex; align-items:center; justify-content:space-between; flex-shrink:0;">
        ...
        <div style="text-align:right;">
          <span style="font-size:11px; font-weight:800; color:var(--color-text-tertiary); text-transform:uppercase; letter-spacing:0.05em;">Total Rese\xF1as</span>
  ...
  `;
  showModal({
    title: `Rese\xF1as de ${u.displayName || "Usuario"}`,
  ...
  if (count > 0) {
    setTimeout(async () => {
      const fetchPromises = sortedRatings.map(async (r, idx) => {
        if (!r.orderId || r.userName) return;
        try {
          ...
          if (orderSnap.exists()) {
            ...
            if (el) {
              ...
              if (u.isDelivery) {
                ...
                if (orderData.driverName) {
                  ...
                  authorText = `Rese\xF1a de Pedido #${r.orderId.slice(-4).toUpperCase()}`;
                }
              }
            ...
            const el = document.getElementById(`review-author-${r.orderId}-${idx}`);
            if (el) el.innerHTML = "Rese\xF1a An\xF3nima";
          }
        } catch (err) {
...
async function showApplicationDetailsModal(u, users) {
  const { showModal: showModal2, closeModal, showConfirm: showConfirm2 } = await import("../../components/modal.js");
  ...
  modalEl.innerHTML = `
    <div style="text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; margin-bottom: 8px;">
      ...
      <h3 style="font-family: var(--font-display); font-size: 19px; font-weight: 900; color: var(--color-text-primary); margin: 0;">Detalle de Postulaci\xF3n</h3>
      <p style="font-size: 12px; color: var(--color-text-secondary); margin: 0;">
        Revis\xE1 los datos y archivos adjuntos del postulante.
      </p>
    </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 12px; background: var(--color-surface); border: 1px solid var(--color-border-light); border-radius: 20px; padding: 16px;">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div>
          <div style="font-size: 9px; font-weight: 900; color: var(--color-text-tertiary); text-transform: uppercase; letter-spacing: 0.5px;">Tel\xE9fono</div>
          <div style="font-size: 14px; font-weight: 700; color: var(--color-text-primary); margin-top: 2px;">${app.phone || u.phone || "Sin tel\xE9fono"}</div>
        </div>
        ${app.phone || u.phone ? `
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
        <div>
          <div style="font-size: 9px; font-weight: 900; color: var(--color-text-tertiary); text-transform: uppercase; letter-spacing: 0.5px;">Veh\xEDculo</div>
    ...
    <div style="display: flex; flex-direction: column; gap: 12px;">
      ...
      ${app.seguroUrl ? `
        <div style="background: var(--color-surface); border: 1px solid var(--color-border-light); border-radius: 18px; padding: 12px; display: flex; flex-direction: column; gap: 6px;">
          <div style="font-size: 10px; font-weight: 900; color: var(--color-text-secondary); text-transform: uppercase; letter-spacing: 0.5px;">Seguro del Veh\xEDculo</div>
  ...
  modalEl.querySelector("#approve-app-btn").onclick = () => {
    showConfirm2({
      ...
      message: `\xBFConfirm\xE1s la aprobaci\xF3n de <b>${app.fullName || u.displayName}</b> como repartidor oficial?`,
      onConfirm: async () => {
        const { runTransaction, doc: fDoc } = await import("firebase/firestore");
        await runTransaction(db, async (t) => {
          const sRef = fDoc(db, "settings", "delivery");
          ...
          t.set(notificationRef, {
            ...
            title: "\u2705 \xA1Postulaci\xF3n Aprobada!",
            body: "\xA1Felicitaciones! Tu postulaci\xF3n como repartidor ha sido aprobada.",
            status: "unread",
            url: "#/profile",
            createdAt: /* @__PURE__ */ new Date()
          });
        });
        showToast("\xA1Repartidor aprobado correctamente!", "success");
        location.reload();
      }
    });
  };
  modalEl.querySelector("#reject-app-btn").onclick = () => {
    showConfirm2({
      title: "Rechazar Postulaci\xF3n",
      message: `\xBFConfirm\xE1s el rechazo de la solicitud de <b>${app.fullName || u.displayName}</b>?`,
      danger: true,
      onConfirm: async () => {
        await updateDoc2(doc2(db, "users", u.uid), {
      ...
      <div style="display:flex; flex-direction:column; gap:8px;">
        ${pending.map((u) => `
          <div class="pending-trip-request-item" data-uid="${u.uid}" style="background:var(--color-surface); border:1px solid var(--color-border-light); padding:10px 14px; border-radius:16px; display:flex; align-items:center; gap:12px; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.borderColor='rgba(59,130,246,0.3)'" onmouseout="this.style.borderColor='var(--color-border-light)'">
            ...
            <div style="flex:1; min-width:0;">
              <div style="font-weight:800; font-size:13.5px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--color-text-primary);">${u.displayName || u.email}</div>
              <div style="font-size:11px; color:var(--color-text-tertiary); margin-top:2px;">Ver detalles del veh\xEDculo y documentos</div>
            </div>
...
async function showTripApplicationDetailsModal(u, users) {
  ...
  modalEl.innerHTML = `
    <div style="text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; margin-bottom: 8px;">
      <img src="${u.photoURL || "/logo.png"}" style="width: 58px; height: 58px; border-radius: 18px; object-fit: cover; border: 2.5px solid var(--color-bg-secondary); box-shadow: var(--shadow-sm);" referrerpolicy="no-referrer" />
      <h3 style="font-family: var(--font-display); font-size: 19px; font-weight: 900; color: var(--color-text-primary); margin: 0;">Detalle de Postulaci\xF3n (Chofer)</h3>
      <p style="font-size: 12px; color: var(--color-text-secondary); margin: 0;">
        Revis\xE1 los datos del veh\xEDculo y archivos adjuntos del postulante a chofer.
      </p>
    </div>
  ...
  modalEl.querySelector("#approve-trip-btn").onclick = () => {
    showConfirm2({
      ...
      message: `\xBFConfirm\xE1s la aprobaci\xF3n de <b>${app.fullName || u.displayName}</b> como chofer oficial para viajes de pasajeros?`,
      onConfirm: async () => {
        const { runTransaction, doc: fDoc } = await import("firebase/firestore");
        await runTransaction(db, async (t) => {
          const sRef = fDoc(db, "settings", "delivery");
          ...
          if (!nId) {
            const nextId = (sSnap.exists() ? sSnap.data().lastDeliveryId || 1e3 : 1e3) + 1;
          ...
          t.set(notificationRef, {
            ...
            title: "\u2705 \xA1Postulaci\xF3n Aprobada!",
            body: "\xA1Felicitaciones! Tu postulaci\xF3n como chofer de viajes ha sido aprobada.",
            status: "unread",
            url: "#/home",
            createdAt: /* @__PURE__ */ new Date()
          });
        });
        showToast("\xA1Chofer aprobado correctamente!", "success");
        location.reload();
      }
    });
  };
  modalEl.querySelector("#reject-trip-btn").onclick = () => {
    showConfirm2({
      title: "Rechazar Postulaci\xF3n de Chofer",
      message: `\xBFConfirm\xE1s el rechazo de la solicitud de chofer de <b>${app.fullName || u.displayName}</b>?`,
      danger: true,
      onConfirm: async () => {
        await updateDoc2(doc2(db, "users", u.uid), {
          tripStatus: "rejected",
        ...
        await updateDoc2(doc2(db, "trip_applications", u.uid), { status: "rejected" });
...
async function showAwardPointsModal(targetUser, allUsers, adminUser) {
  const { showModal: showModal2, closeModal } = await import("../../components/modal.js");
  const { showToast: showToast2 } = await import("../../components/toast.js");
  const { doc: doc2, collection: collection2, runTransaction } = await import("firebase/firestore");
    ...
    <div style="padding: 24px 20px; color: var(--color-text-primary); font-family: var(--font-body); display: flex; flex-direction: column; gap: 16px;">
      <div style="text-align: center; display: flex; flex-direction: column; align-items: center; gap: 8px; margin-bottom: 8px;">
        ...
        <p style="font-size: 12px; color: var(--color-text-secondary); margin: 0; line-height: 1.4; max-width: 260px;">
          Otorg\xE1 puntos administrativos directamente a la cuenta de <strong>${targetUser.displayName || "este usuario"}</strong> sin l\xEDmites.
        </p>
      </div>
  ...
  showModal2({
    ...
    onOpen: () => {
      ...
      execBtn?.addEventListener("click", async () => {
        ...
        const reason = reasonInput.value.trim() || "Cr\xE9dito administrativo de cortes\xEDa";
        ...
        try {
          await runTransaction(db, async (transaction) => {
            const userRef = doc2(db, "users", targetUser.uid);
            ...
            const transRef = doc2(collection2(db, "points_transactions"));
            ...
            const notificationRef = doc2(collection2(db, "users", targetUser.uid, "notifications"));
            transaction.set(notificationRef, {
              type: "points_received",
              title: "\u{1F381} \xA1Recibiste GO Points de cortes\xEDa!",
              body: `El equipo de GoDelivery te otorg\xF3 ${amount} GO Points de regalo. Motivo: ${reason}`,
              status: "unread",
              url: "#/profile",
              createdAt: /* @__PURE__ */ new Date()
            });
          ...
          showToast2(`\xA1Cargaste ${amount} GO Points con \xE9xito!`, "success");
          closeModal();
        } catch (error) {
          console.error("Award points failed:", error);
          showToast2(error.message || "Error al otorgar los puntos", "error");
...
function showEditVehicleModal(u, onSuccess) {
  ...
  modalEl.innerHTML = `
    <h3 style="margin: 0; font-family: var(--font-display); font-size: 18px; font-weight: 900; color: var(--color-text-primary);">Editar Veh\xEDculo de ${u.displayName || "Chofer"}</h3>
    
    <div style="display: flex; flex-direction: column; gap: 4px;">
      <label style="font-size: 11px; font-weight: 800; color: var(--color-text-tertiary); text-transform: uppercase;">Tipo de Veh\xEDculo</label>
      <select id="edit-veh-type" style="width: 100%; height: 42px; border-radius: 10px; border: 1.5px solid var(--color-border-light); background: var(--color-surface); color: var(--color-text-primary); font-size: 13px; font-weight: 700; padding: 0 12px; outline: none; font-family: inherit;">
        ...
        <option value="Moto" ${(u.vehicleType || "").toLowerCase() === "moto" ? "selected" : ""}>\u{1F3CD}\uFE0F Moto</option>
        <option value="Auto" ${(u.vehicleType || "").toLowerCase() === "auto" ? "selected" : ""}>\u{1F697} Auto</option>
      </select>
    </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 4px;">
      <label style="font-size: 11px; font-weight: 800; color: var(--color-text-tertiary); text-transform: uppercase;">N\xFAmero de Patente</label>
  ...
  saveBtn.onclick = async () => {
    ...
    try {
      ...
      const userRef = fDoc(db2, "users", u.uid);
      ...
      import("../../components/toast.js").then((m) => m.showToast("Veh\xEDculo actualizado con \xE9xito", "success"));
      const { closeModal } = await import("../../components/modal.js");
      closeModal();
    } catch (err) {
      console.error("Error saving user vehicle details:", err);
      import("../../components/toast.js").then((m) => m.showToast("Error al guardar veh\xEDculo", "error"));
